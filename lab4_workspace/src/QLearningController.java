...
public class QLearningController extends Controller {
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			 ...
			 */
			action_counter++;
			if (new_state.equals(previous_state) && action_counter < REPEAT_ACTION_MAX) { return;
			}
			...
			if (previous_state != null) {
				...
				// Update Q function
				double currentValue = Qtable.get(prev_stateaction);
				double learningRate = alpha(Ntable.get(prev_stateaction));
				double temporalDiff = previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state) - currentValue;

				double qValue = currentValue + learningRate * temporalDiff;
				...
				if (print_counter % 10 == 0) {
					System.out.println("ITERATION: " + iteration + " SENSORS: a=" + df.format(angle.getValue()) + " vx="
							+ df.format(vx.getValue()) +
							" vy=" + df.format(vy.getValue()) + " P_STATE: " + previous_state + " P_ACTION: "
							+ previous_action +
							" P_REWARD: " + df.format(previous_reward) + " P_QVAL: "
							+ df.format(Qtable.get(prev_stateaction)) + " Tested: "
	...
	public double alpha(int num_tested) {
		 ...
		 * therefore make the agent behavior converge to
		 * to a solution faster, but if the state space is not properly explored at that
		 * point the resulting behavior may be poor.
